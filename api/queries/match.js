module.exports = {
  ...
  GET_PLAYER_SONG_SCORES: `SELECT mps.player_id, mps.song_id, mps.score, mps.win, p.username as player_gamertag, s.title as song_title, s.artist as song_artist
    FROM match_x_player_x_song mps
    LEFT JOIN player p ON mps.player_id = p.id
    LEFT JOIN song s ON mps.song_id = s.id
    WHERE mps.match_id = ?
  ...
  CREATE_MATCH: `INSERT INTO \`match\` (event_id, winner_id, created_by) 
    VALUES (?, ?, ?)`,
  CREATE_MATCH_PLAYER_SONG: `INSERT INTO match_x_player_x_song (match_id, player_id, song_id, score, win, created_by) 
    VALUES (?, ?, ?, ?, ?, ?)`,
